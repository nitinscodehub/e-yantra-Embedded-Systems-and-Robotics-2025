...
void lcd_wr_char(char row, char column, char alpha_num_char)
{
	...
	temp = alpha_num_char;
	temp = (temp & 0xF0);								// upper 4 bits of data	
	lcd_data_port_reg &= 0x0F;				// flushes upper half of PortD to 0, but retains lower half of PortD
	lcd_data_port_reg |= temp;
...
void lcd_char(char alpha_num_char)
{
	char temp;
...
/* Function Name: LCD_Custom_Char
 * LCD_Custom_Char	
 * Input: loc, msg => where you want the custom character to be positioned in CGRAM     
 * Output:None
